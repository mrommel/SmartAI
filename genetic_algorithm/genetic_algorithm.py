...
class GeneticAlgorithm():
	...
	def generate_binary_population(self, size, genome_length):
		self.population = np.array(
			[[not not x for x in line] for line in np.random.randint(0, 2, (size, genome_length))])
	...
	def _generate_children(self, parent_pair):
		...
		if self.single_point_cross_over:
			cut_after = np.random.randint(0, len(self.population[0]) - 1)
			# Concatenate returns two arrays joined together as a new array
			return (np.concatenate((parent1[0:cut_after + 1], parent2[cut_after + 1:])),
			        np.concatenate((parent2[0:cut_after + 1], parent1[cut_after + 1:])))
		else:
			# Uniform crossover:
